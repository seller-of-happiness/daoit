...
import { IExistingFile } from '@/refactoring/modules/files/types/IExistingFile'
...
const modalHeader = computed<string>(() => {
    if (feedbackStore.isDataLoading) return 'Загрузка...'
...
const modalSaveButtonText = computed<string>(() => {
    return route.params.id ? 'Сохранить' : 'Создать новое НС'
})
...
const getValidationMessage = (): string => {
    ...
    if (fieldValidation.adverseEventValidation.location) errors.push('не указано место события')
...
function validateAdverseEvent(adverseEvent: ICreateAdversePayload): boolean {
    ...
    const hasManualLocation =
        typeof adverseEvent.location === 'string' && adverseEvent.location.trim() !== ''
    const hasTreeLocation =
        !!adverseEvent.block?.id || !!adverseEvent.floor?.id || !!adverseEvent.room?.id
    if (!hasManualLocation && !hasTreeLocation) {
        responsibilityEntries.fieldValidation.adverseEventValidation.location = true
        isValid = false
    }
...
async function onFormSubmit() {
    ...
    try {
        ...
        if (isOnlyNewEntryAdded) {
            ...
            await responsibilityEntries.getResponsibilityEntries({ event_id: id })
            await apiStore.fetchAdverseEventById({ id })
            await syncAdverseFiles(id)
            return
        }
...
const isFormTabError = computed(
    () =>
        ...
        responsibilityEntries.fieldValidation.adverseEventValidation.location ||
...
onMounted(async () => {
    ...
    if (id) {
        ...
        // 2) Пытаемся загрузить мероприятия
        try {
            await responsibilityEntries.getResponsibilityEntries({ event_id: id })
        } catch (e) {
            await router.replace({ name: ERouteNames.ADVERSE_EVENTS })
...
<template>
    ...
    >
        <template #header>
            <p v-if="tooltip" class="p-dialog-title" v-app-tooltip.bottom="tooltip">
                {{ modalHeader }}
            </p>
            <p v-else class="p-dialog-title">
                {{ modalHeader }}
            </p>
        </template>
        ...
        <Form @submit="onFormSubmit" @entry-added="onFormSubmit">
          ...
          ></span>
                                ...
                                <!-- Место события -->
                                <div class="col-span-12 md:col-span-4 min-w-0 flex items-center flex-col justify-center">
                                    <label class="font-bold label-required mb-3">Место события</label>
                                    ...
                                    <Button
                                        ...
                                        :disabled="!canEditFormGeneral"
                                        :class="{ 'field-error': fieldValidation.adverseEventValidation.location }"
                                    >
                                    ...
                                    <Button
                                        ...
                                        :disabled="!canEditFormGeneral"
                                        :class="{
          'field-error': fieldValidation.adverseEventValidation.location,
          'field-error-text': fieldValidation.adverseEventValidation.location,
        }"
                                    >
                                        {{
                                            fieldValidation.adverseEventValidation.location
                            ...
                            <div class="overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
                                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                    ...
                                    <tr
                                        ...
                                        :key="p.key || p.employee?.id || p.full_name || p.comment || 'last-row'"
            ...
            <span v-if="p.birth_date">
              {{ formatDateOnly(p.birth_date) }}
                                            ...
                                            <span v-else-if="p.employee?.birth_date">
              {{ formatDateOnly(p.employee.birth_date) }}
            </span>
                                            <span v-else class="text-gray-500 italic">---- </span>
                                        </td>
            ...
            <span v-if="p.phone_number">
              {{ formatPhoneNumber(p.phone_number) }}
                                            ...
                                            <span v-else-if="p.employee?.phone_number">
              {{ formatPhoneNumber(p.employee.phone_number) }}
            </span>
                                            <span v-else class="text-gray-500 italic">---- </span>
                                        </td>
                                ...
                                >
                                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                        ...
                                        <tr
                                            ...
                                            :key="data.key || data.employee?.id || data.full_name || data.comment || 'row'"
                                            class="odd:bg-white even:bg-gray-50/50 dark:odd:bg-gray-900 dark:even:bg-gray-900/60 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
                                        >
                                            <!-- Тип участника -->
                                            <td class="px-4 py-3 align-top">
                    ...
                    {{ formatDateOnly(data.birth_date) }}
                                            ...
                                            <!-- Телефон (ровно как у тебя, только безопасная строка для TS) -->
                                            <td class="px-4 py-3 align-top">
                                                <div class="flex flex-col">
                                                    <template v-if="data.participant_type === 'other'">
                                                        <span class="text-gray-500 italic"> ---- </span>
                                                    ...
                                                    <template v-else>
                                                        {{
                                                            data.phone_number || data.employee?.phone_number
                                                                ? formatPhoneNumber(String(data.phone_number ?? data.employee?.phone_number))
                                                                : ''
                                                        }}
                                                    </template>
                                                </div>
