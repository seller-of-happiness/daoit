...
const { current } = storeToRefs(news)
...
const submit = async () => {
    ...
    try {
        ...
        if (isEdit.value) {
            const id = Number(route.params.id)
            await news.updateNews(id, payload as any)
            await router.push({ name: ERouteNames.NEWS_DETAIL, params: { id } })
        } else {
            const created = await news.createNews(payload as any)
            await router.push({ name: ERouteNames.NEWS_DETAIL, params: { id: created?.id } })
        }
    } catch (e) {
...
const goBack = () => {
    if (isEdit.value && current.value?.id) {
        router.push({ name: ERouteNames.NEWS_DETAIL, params: { id: current.value.id } })
    } else {
        router.push({ name: ERouteNames.NEWS })
    }
}
</script>
