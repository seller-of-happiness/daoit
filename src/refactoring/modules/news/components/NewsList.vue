...
const loadMore = () => {
    if (tab.value === 'all' && nextCursor.value) void news.fetchNews(nextCursor.value)
    if (tab.value === 'my' && myNextCursor.value) void news.fetchMyNews(myNextCursor.value)
    if (tab.value === 'dept' && departmentNextCursor.value)
        void news.fetchDepartmentNews(departmentNextCursor.value)
}
const open = (id: number) => {
    router.push({ name: ERouteNames.NEWS_DETAIL, params: { id } })
}
const toggleLike = (post: any) => {
    void (post.is_liked ? news.unlike(post.id) : news.like(post.id))
}
const switchTab = (t: 'all' | 'my' | 'dept') => {
    tab.value = t
}
const createPost = () => {
    router.push({ name: ERouteNames.NEWS_CREATE })
}
