...
onMounted(async () => {
    ...
    news.incrementViewsLocally(id)
    await news.fetchNewsById(id)
    if (!items.value?.length) void news.fetchNews()
})
...
const editPost = () => {
    if (!post.value) return
    router.push({ name: ERouteNames.NEWS_EDIT, params: { id: post.value.id } })
}
const removePost = async () => {
    if (!post.value) return
    await news.deleteNews(post.value.id)
...
const handleFileChange = async (e: Event) => {
    ...
    await news.uploadAttachment(post.value.id, input.files[0])
    input.value = ''
}
const deleteAttachment = async (attachmentId: number) => {
    if (!post.value) return
    await news.deleteAttachment(post.value.id, attachmentId)
}
