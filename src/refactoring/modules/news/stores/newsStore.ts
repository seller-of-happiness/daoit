...
import type { ICreateOrUpdateNewsPayload } from '@/refactoring/modules/news/types/ICreateOrUpdateNewsPayload'
...
export const useNewsStore = defineStore('newsStore', {
    ...
    actions: {
        ...
        async like(id: number) {
            ...
            const item = this.items.find((i: INewsPost) => i.id === id)
            if (item) {
                item.is_liked = true
                item.likes_count += 1
            }
            ...
            const myItem = this.myItems.find((i: INewsPost) => i.id === id)
            if (myItem) {
                myItem.is_liked = true
                myItem.likes_count += 1
            }
            const deptItem = this.departmentItems.find((i: INewsPost) => i.id === id)
            if (deptItem) {
        ...
        async createNews(payload: ICreateOrUpdateNewsPayload) {
            ...
            return res.data as INewsPost
        },
        async updateNews(id: number, payload: ICreateOrUpdateNewsPayload) {
            ...
            const updateInList = (list: INewsPost[]) => {
        ...
        async deleteNews(id: number) {
            ...
            this.items = this.items.filter((i: INewsPost) => i.id !== id)
            this.myItems = this.myItems.filter((i: INewsPost) => i.id !== id)
            this.departmentItems = this.departmentItems.filter((i: INewsPost) => i.id !== id)
        ...
        incrementViewsLocally(id: number) {
            const incrementInList = (list: INewsPost[]) => {
                const item = list.find((i: INewsPost) => i.id === id)
