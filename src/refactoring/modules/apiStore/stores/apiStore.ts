...
export const useApiStore = defineStore('apiStore', {
    ...
    actions: {
        ...
        async applyAdverseEventRealtime(payload: IRealtimePayload<IAdverseEvent>): Promise<void> {
            ...
            if (!payload?.event_type || !payload?.object) return

            const ev = payload.object
        ...
        async createAdverseEvent(payload: ICreateAdversePayload): Promise<IAdverseEvent> {
            ...
            try {
                ...
                feedbackStore.showToast({
                    type: 'error',
                    title: 'Ошибка',
                    message: `Не удалось создать НС: ${error}`,
                    time: 7000,
                });
                throw error;
            } finally {
        ...
        async getAdverseEventParticipants(payload: { eventId: number; id: number }): Promise<any> {
            ...
            feedbackStore.isDataLoading = true // включаем индикатор загрузки
            try {
                const response = await axios.get(
                ...
                feedbackStore.isDataLoading = false // выключаем индикатор загрузки
            }
        },
