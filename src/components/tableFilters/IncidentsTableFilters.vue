...
const searchInput = ref(apiStore.filters.search) // Локальное значение поискового запроса
...
const applyQuickPreset = (preset: 'my_active' | 'current_month' | 'prev_month' | 'high_risk') => {
    ...
    // Тоггл «Высокий риск»
    if (preset === 'high_risk') {
        const wasActive = apiStore.filters.is_high_risk
        apiStore.resetFilters()
        apiStore.filters.is_high_risk = !wasActive
    ...
    if (preset === 'my_active') {
        const wasActive = apiStore.filters.is_active
        apiStore.resetFilters()
        apiStore.filters.is_active = !wasActive
        suppressFilterWatch.value = false
        return
    }
...
// Синхронизация локального searchInput с хранилищем
watch(
    () => apiStore.filters.search,
    (val) => {
            ...
            <div v-show="!isFiltersCollapsed" class="filters-panel__content grid grid-cols-12 gap-4">
                ...
                <div class="col-span-12 flex flex-col lg:flex-row gap-4">
                    ...
                    <MultiSelect
                        ...
                        display="chip"
                        :loading="feedbackStore.isDataLoading"
                ...
                <div class="filters-panel__chips col-span-12 flex flex-wrap gap-2">
                    ...
                    <Tag v-if="apiStore.filters.number!==null" severity="secondary" :value="`№ ${apiStore.filters.number}`" />
