...
/**
 * НС завершено → вся карточка read-only (даже если задача не completed).
 * Возвращает: boolean
 * Зависимости: apiStore.currentAdverseEvent.status
 */
...
/**
 * Отправка отчёта о выполнении:
 * - Валидация: отчёт не пустой → иначе ставит reportError и выходит.
 * - Вызывает completeResponsibilityEntry({ event_id, entry_id, completion_report, files? }).
 * - При переданном event_id — перезагружает НС (fetchAdverseEventById).
 * - Очищает completionReport и files по успеху.
 * Игнорирует вызов, если задача/НС завершены.
 * Параметры: event_id?:number
...
/**
 * Берёт задачу в работу и обновляет данные НС.
 * Вызовы:
 * - responsibilityEntries.takeResponsibilityEntry({ event_id, entry_id })
 * - apiStore.fetchAdverseEventById({ id: event_id })
 * Параметры: event_id:number, entry_id:number
 * Возврат: Promise<void>
 */
...
/**
 * Назначение:
 * - Вернуть нормализованный список файлов, прикреплённых к конкретной записи ответственности текущего НС.
 *
 * Источник данных:
 * - apiStore.currentAdverseEvent.responsibility_entries
 * - Поиск записи по props.entryId.
 *
 * Логика:
 * - Берёт responsibility_entries → ищет запись с id === props.entryId.
 * - Безопасно приводит entry.files к массиву (иначе — пустой массив).
 * - Для каждого элемента извлекает:
 *   • id: number — берётся из f.id (строка приводится к числу, отбрасываются NaN).
 *   • file: string — URL; берётся из f.file (если это строка) или из f.file.url.
 * - Пропускает элементы без валидных id или file.
 *
 * Возвращает:
 * - IExistingFile[] — массив объектов вида { id: number; file: string }.
 *
 * Особенности:
 * - Не бросает ошибок при отсутствующих полях/неожиданных форматах, просто возвращает [].
 * - Реактивно пересчитывается при изменении currentAdverseEvent/responsibility_entries.
 */
const currentEntryFiles = computed<IExistingFile[]>(() => {
    const ev = apiStore.currentAdverseEvent as any
...
<template>
    ...
    >
        ...
        >
            ...
            <!-- ЛЕВАЯ ЧАСТЬ: Постановщик · Исполнитель · Срок -->
            <div class="flex items-center gap-1 min-w-0">
                <!-- Постановщик -->
                <span class="truncate max-w-[22ch]">Постановщик -</span>
                <button
                    type="button"
                    class="performer-task__meta inline-flex items-center min-w-0 text-sm cursor-pointer"
                    @click.stop="openChat(supervisor?.id)"
                    title="Постановщик задачи"
                >
                    <i class="pi pi-user text-xs mr-1"></i>
                    <span class="truncate max-w-[22ch]">{{ getFullName(supervisor ?? {}) }}</span>
                </button>

                <!-- Разделитель -->
                <span v-if="responsibleEmployee" class="text-surface-400 ms-2 me-2">·</span>

                <!-- Исполнитель -->
                <span class="truncate max-w-[22ch]">Исполнитель -</span>
                <button
    ...
    {{ taskStatus }}
        ...
        <div v-else-if="isCompletedTask && !isExpanded" class="text-sm text-surface-500 cursor-pointer" @click="isCompletedTask && toggleExpand()">
            Задача завершена. Нажмите для просмотра деталей.
        </div>
    </section>
</template>
