...
const myActiveSupportCount = computed(() => {
    return statsDashboard.value?.support_service?.active ?? 0
})
...
const departmentActiveSupportCount = computed(() => {
    return statsDashboard.value?.support_service?.department_active ?? 0
})
...
const currentMonthSupport = computed(() => {
    return statsDashboard.value?.support_service?.current_month ?? 0
})
...
const prevMonthSupport = computed(() => {
    return statsDashboard.value?.support_service?.prev_month ?? 0
})
...
const handleSupportClick = async () => {
    supportService.resetFilters()
    supportService.filters.is_active = true // Устанавливаем фильтр по активным
    await router.push({ name: ERouteNames.SUPPORT_SERVICE })
    await supportService.fetchFilteredSupportServices()
}
</script>
